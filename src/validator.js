const validator = {
  isValid: function (cardNumber) {
    ...
    let sum = 0; //verifica que la suma sea 0 
    ...
    for (let i = 0; i < numCard.length; i++) {//inicia en la posición 0, aplicammos a todo el arreglo, de uno en uno
      if (i % 2 === 1) {//Si el residuo de la posicion de cada arreglo es =1(impar)
        let multiply = numCard[i] * 2  //Se declara una varibale que tome el valor de esa posición y *2
        console.log(multiply)
        if (multiply > 9) { //Si ese valor es mayor a 9
          let resultadoFinal = multiply - 9// restamos 9 (Lo mismo que sumar cifra de dos digitos)
          sum += resultadoFinal //a mi variable (sum) le sumo los numeros que cumplen la condición anterior
        } else {
          sum += multiply;//si no es >9 y esta en posición par, entonces solo se duplica y se suma a la variable (sum)
  ...
  maskify: function(cardNumber) {
    ...
    if (longArray <= 4) { //Si la longitud del arreglo es igual o menor a 4
        return miArray //retornamos el arreglo 
    } else {
